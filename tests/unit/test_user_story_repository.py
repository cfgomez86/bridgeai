...
def test_find_by_requirement_and_analysis_returns_story():
    repo = make_repo()
    repo.save(make_story("s1", requirement_id="req-x", analysis_id="ana-x"))
    found = repo.find_by_requirement_and_analysis("req-x", "ana-x")
    assert found is not None
    assert found.id == "s1"
...
def test_find_by_requirement_and_analysis_returns_none_if_missing():
    repo = make_repo()
    assert repo.find_by_requirement_and_analysis("req-x", "ana-x") is None


def test_find_by_requirement_and_analysis_does_not_match_partial():
    repo = make_repo()
    repo.save(make_story("s1", requirement_id="req-x", analysis_id="ana-x"))
    assert repo.find_by_requirement_and_analysis("req-x", "ana-OTHER") is None
